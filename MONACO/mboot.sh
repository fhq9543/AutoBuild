...
svn_path=http://172.20.3.78/svn/Android/800C/sourcenew/MBoot/
svn_username=fae
svn_password=123456
code_path=$work_path/mboot
image_path=$work_path/mboot/sboot/out
build_flow()
{
    chmod +x -R mboot && \
    cd mboot/sboot && \
    cp configs/monaco/.config.monaco.android.068d.rom_emmc.security.tee .config && \
    echo EY > FILE && \
    make menuconfig < FILE && \
    make clean && \
    make 
}
